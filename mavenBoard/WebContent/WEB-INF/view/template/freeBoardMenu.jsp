...
<body>
	<c:if test="${sessionScope.user_id!=null}">
		...
		<div>
			<ul>
				<li class="menu"><a>메인</a><a href="./main.ino">(이동)</a>
					<ul>
						<c:forEach var="nRow1" items="${sessionScope.id_authoritylist}">
							<c:forEach var="nRow2" items="${sessionScope.id_authoritylist}">
								<c:if
									test="${nRow1.DEPT == '2' and nRow1.OBJECTID==nRow2.OBJECTID}">
									<li class="menu"><a>${nRow1.OBJECTNM }</a><a
										href="${nRow1.OBJECTID}.ino">(이동)</a> <c:forEach var="nRow3"
											items="${sessionScope.id_authoritylist}">
											<c:if
												test="${nRow3.DEPT == '3' and nRow3.HIGH_OBJ==nRow1.OBJECTID}">

												<c:choose>
													<c:when test="${nRow3.HIGH_OBJ == 'OBJ120'}">
														<ul>

															<li value="${nRow3.OBJECTID }"><a
																value="${nRow3.OBJECTID }">${nRow3.OBJECTNM }</a><a
																href="${nRow3.OBJECTID }.ino">(이동)</a></li>
